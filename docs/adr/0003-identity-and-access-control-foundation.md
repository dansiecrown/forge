...
## Addendum — 2026-07-24: multi-organization identity fix (Architecture Lock)

An Engineering Readiness Review found that `POST /users/invitations` rejected any email that already existed *anywhere on the platform*, not just within the target organization — meaning a person who already had an account in one organization could never be invited into a second one. This directly contradicted this platform's core, already-modeled data shape: identity (`User`) is global, membership (`Membership`) is per-organization, and `docs/database-design.md` explicitly documents "one user can be a student in several organizations." The schema already supported this correctly; the bug was entirely in the service layer.

**Decision: identity and membership are formally separated in the invitation flow.** `UsersService.invite()` no longer rejects an existing email — it now returns the existing `User` row (`isNewUser: false`) instead of creating a duplicate, and issues no new credential or "set your password" email for that case, since one already exists. `UsersController.invite()` always proceeds to `MembershipsService.inviteIntoOrganization()` either way, which now checks for an existing membership row for that (organization, user) pair before creating one — via a new `MembershipsRepository.findByOrganizationAndUser` lookup on the existing `(organization_id, user_id)` unique constraint — and rejects with `409 ALREADY_MEMBER` if one already exists, instead of the previous unhandled `500` a race against that constraint would have produced. Brand-new identities go through the exact same flow as before this fix (new `User` row, password-reset-token-based set-password email, membership created as `invited`); this addendum changes nothing about that path.

No schema change was needed or made — `Membership` already correctly models one user across many organizations. The response shape for `POST /users/invitations` gained a second `status` value: `"sent"` for a new identity (email dispatched) and `"added"` for an existing identity reused into a new organization (no email dispatched by this endpoint). `docs/api-specification.md` §4.2 is updated accordingly.

Two related membership-lifecycle questions were identified but deliberately **not** resolved here, per this milestone's "document, don't guess" instruction — see `docs/KNOWN_TECHNICAL_DEBT.md`:
- `memberships`' unique constraint on `(organization_id, user_id)` is not partial on status, so a *former* member (status `ended`) cannot currently be re-invited into the same organization — that call now correctly returns `409 ALREADY_MEMBER` rather than a raw `500`, but re-activating an ended membership is not implemented, since "what should happen to an ended membership's history" is a product decision, not a bug fix.
