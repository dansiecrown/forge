...
describe('AcademiesService', () => {
  it('treats an academy from another organization as not found', async () => {
    ...
    const service = new AcademiesService(repository as AcademiesRepository, fakeAuditLog());

    await expect(service.get({ organizationId: 'org-2' }, 'academy-1')).rejects.toMatchObject({
